...
function greenlet_do_archive_header() {
	...
	if ( isset( $archive_title ) ) {
		printf( '<h1 %s>', greenlet_attr( 'archive-title' ) );
		printf( __( '%s', 'greenlet' ), $archive_title );
		echo '</h1>';
	}
	if ( isset( $archive_description ) ){
		printf( __( '%s', 'greenlet' ), $archive_description );
	}
}
...
function greenlet_do_entry_header() {
	...
	if ( is_singular() ) {
		printf( '<h1 %s>', greenlet_attr( 'entry-title' ) );
		the_title();
		echo '</h1>';
	} else {
		printf( '<h2 %s>', greenlet_attr( 'entry-title' ) ); ?>
	...
	if ( get_post_type() === 'post' ) {
		...
		if ( is_sticky() ) {
			printf(
				...
				greenlet_attr( 'meta-featured-post list-inline-item' ),
				// file_get_contents( IMAGES_DIR . '/icons/pin-icon.svg' ),
				greenlet_get_file_contents( IMAGES_DIR . '/icons/pin-icon.svg' ),
				__( 'Featured', 'greenlet' )
			);
		}
		...
		printf(
			...
			greenlet_attr( 'meta-author list-inline-item' ),
			// file_get_contents( IMAGES_DIR . '/icons/user-icon.svg' ),
			greenlet_get_file_contents( IMAGES_DIR . '/icons/user-icon.svg' ),
		...
		printf(
			...
			greenlet_attr( 'meta-date list-inline-item' ),
			// file_get_contents( IMAGES_DIR . '/icons/date-icon.svg' ),
			greenlet_get_file_contents( IMAGES_DIR . '/icons/date-icon.svg' ),
			get_the_date()
		);
		...
		if ( $category_list ) {
			printf(
				...
				greenlet_attr( 'meta-categories list-inline-item' ),
				// file_get_contents( IMAGES_DIR . '/icons/folder-icon.svg' ),
				greenlet_get_file_contents( IMAGES_DIR . '/icons/folder-icon.svg' ),
				$category_list
			);
		}
		...
		if ( $tag_list ) {
			printf(
				...
				greenlet_attr( 'meta-tags list-inline-item' ),
				// file_get_contents( IMAGES_DIR . '/icons/tag-icon.svg' ),
				greenlet_get_file_contents( IMAGES_DIR . '/icons/tag-icon.svg' ),
				$tag_list
			);
		}
		...
		// Comments link.
		if ( comments_open() ) {
			printf( '<li %s><span class="comment-icon">%s</span> ',
				greenlet_attr( 'meta-reply list-inline-item' ),
				// file_get_contents( IMAGES_DIR . '/icons/comment-icon.svg' )
				greenlet_get_file_contents( IMAGES_DIR . '/icons/comment-icon.svg' )
			);
		...
		if ( is_user_logged_in() && current_user_can( 'edit_posts' ) ) {
			printf( '<li %s><span class="dashicons dashicons-edit"></span> ', greenlet_attr( 'meta-edit list-inline-item' ) );
...
function greenlet_do_entry_content() {
	...
	if ( is_single() || is_page() ) {
		...
		if ( ( $show_image == 1 ) && has_post_thumbnail() && ! post_password_required() ) {
		...
		if ( $is_more ) the_content( $more );
		elseif ( greenlet_excerpt_length() != 0 ) {
		 	the_excerpt();
		}
	}
...
function greenlet_do_entry_footer() {
	...
	$name	= of_get_option( 'show_author' )['name']	? of_get_option( 'show_author' )['name']	: null;
	$image	= of_get_option( 'show_author' )['image']	? of_get_option( 'show_author' )['image']	: null;
	$bio	= of_get_option( 'show_author' )['bio']		? of_get_option( 'show_author' )['bio']		: null;
	...
	if ( is_single() && ( $name || $image || $bio ) ) {
		...
		if ( $image ) {
			...
			printf( '<div %s>%s</div>', greenlet_attr( 'author-avatar' ), get_avatar( get_the_author_meta( 'user_email' ), $size ) );
		}

		printf( '<div %s>', greenlet_attr( 'author-description' ) );
			if ( $name ) {
...
function greenlet_paging_nav() {
	...
	if ( $format === 'number' || $format === 'ajax' ) {

		$pages = paginate_links( apply_filters( 'greenlet_pagination_args', array(
	...
	echo apply_filters( 'greenlet_paging_nav', $op, $pages, $pag_attr );
}
...
function greenlet_get_paginated() {
	...
	add_filter( 'post_class', function ( $classes ) { $classes[] = 'post'; return $classes; } );
	...
	//$args['category_name'] = $args_in['category_name'];
	$args = $args_in;
	...
	if ( $wp_query->have_posts() ) {
		...
		$_RESPONSE['posts'] = ob_get_contents();
	    ...
	    $_RESPONSE['no_posts'] = ob_get_contents();
	}
	ob_end_clean();
