...
/**
 * Sanitization for matcher
 *
 * @param array of select option values
 * @return array of sanitized values ('1' or false)
 */
function of_sanitize_matcher( $input, $option ) {
	$output = array();
	if ( is_array( $input ) ) {
		foreach( $option['options'] as $key => $value ) {
			$output[$key] = false;
...
add_filter( 'of_sanitize_matcher', 'of_sanitize_matcher', 10, 2 );
...
function of_sanitize_allowedtags( $input ) {
	//global $allowedtags;
	$output = esc_attr( $input );//wpautop( wp_kses( $input, $allowedtags ) );
	return $output;
}
...
function of_sanitize_typography( $input, $option ) {
	...
	$output['googlefont'] = apply_filters( 'of_sanitize_googlefont', $output['googlefont'] );
	return $output;
}
add_filter( 'of_sanitize_typography', 'of_sanitize_typography', 10, 2 );
add_filter( 'of_sanitize_googlefont', 'of_sanitize_checkbox' );
...
function of_recognized_background_repeat() {
	$default = array(
		'no-repeat' => __( 'No Repeat', 'greenlet' ),
		'repeat-x'  => __( 'Repeat Horizontally', 'greenlet' ),
		'repeat-y'  => __( 'Repeat Vertically', 'greenlet' ),
		'repeat'    => __( 'Repeat All', 'greenlet' ),
		);
...
function of_recognized_background_position() {
	$default = array(
		'top left'      => __( 'Top Left', 'greenlet' ),
		'top center'    => __( 'Top Center', 'greenlet' ),
		'top right'     => __( 'Top Right', 'greenlet' ),
		'center left'   => __( 'Middle Left', 'greenlet' ),
		'center center' => __( 'Middle Center', 'greenlet' ),
		'center right'  => __( 'Middle Right', 'greenlet' ),
		'bottom left'   => __( 'Bottom Left', 'greenlet' ),
		'bottom center' => __( 'Bottom Center', 'greenlet' ),
		'bottom right'  => __( 'Bottom Right', 'greenlet')
		);
...
function of_recognized_background_attachment() {
	$default = array(
		'scroll' => __( 'Scroll Normally', 'greenlet' ),
		'fixed'  => __( 'Fixed in Place', 'greenlet')
		);
...
function of_recognized_font_styles() {
	$default = array(
		'normal'      => __( 'Normal', 'greenlet' ),
		'italic'      => __( 'Italic', 'greenlet' ),
		'bold'        => __( 'Bold', 'greenlet' ),
		'bold italic' => __( 'Bold Italic', 'greenlet' )
		);
