		...
		foreach ( $options as $value ) {
			...
			case 'number':
				...
				if ( isset( $value['settings'] ) ) {
					foreach ( $value['settings'] as $attr => $value ) {
						$output .= ' ' . $attr . '="' . $value . '" />';
					}
				} else $output .= ' />';
				break;
			...
			// Matcher
			case "matcher":
				$output .= '<div class="spinner"></div><div class="matcher">';
				foreach ($value['options'] as $key => $option) {
					$selected = '';
					$label = $option;
					$option = preg_replace('/[^a-zA-Z0-9._\-]/', '', strtolower($key));
					...
					$output .= '<select id="' . esc_attr( $id ) . '" class="of-input" name="' . esc_attr( $name ) . '" style="width:70%;margin:2px 10px;">';
					foreach ($value['selections'] as $key2 => $option2 ) {
						if ( isset($val[$option]) ) {
							$selected = selected( $val[$option], $key2, false );
						}
			...
			case 'typography':
				...
				if ( $typography_options['googlefont'] ) {
					$output .= '<div class="spinner"></div><p class="google-fonts">';
					$output .= '<input class="of-typography of-typography-googlefont" type="checkbox" name="' . esc_attr( $option_name . '[' . $value['id'] . '][googlefont]' ) . '" id="' . esc_attr( $value['id'] . '_googlefont' ) . '" ' . checked( $typography_stored['googlefont'], 1, false ) . '>';
					$output .= '<label for="' . esc_attr( $value['id'] . '_googlefont' ) . '" class="' . esc_attr( $value['id'] ) . '">Google Fonts</label></p>';
				}

				$output .= '<div class="typowrap">';
				$output .= implode( '', $typography_fields );
				$output .= '</div><div class="font-test"></div>';
			...
			if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
				...
				if ( ( $value['type'] != "checkbox" ) && ( $value['type'] != "editor" ) ) {
					$output .= '<div class="explain">' . $explain_value /*wp_kses( $explain_value, $allowedtags)*/ . '</div>'."\n";
				}
				$output .= '</div></div>'."\n";
			}
