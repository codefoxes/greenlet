...
function optionsframework_options() {
	...
	$css_frameworks = array(
		'default' => 'Circle CSS Framework',
		'bootstrap' => 'Bootstrap ( v4.4.1 & stackpath )'
	);

	$separator = array( 'desc' => '<hr>', 'type' => 'info', 'class' => 'no-hidden' );
	...
	$options[] = array(
		...
		'desc' => sprintf( 'How many sidebars you want ro register for main container of page ? ( Not for header or footer. They will be configured in "Header/Footer Layout".)
			Minimum sidebars required are "%s" according to your templates.', $min_sidebars ),
		'id' => "sidebars_qty",
		'class' => "mini",
		'std' => "3",
		'type' => "select",
	...
	$options[] = array(
		...
		'desc' => __( 'Select Layout for your home page or post list page. If you set a "static page" as home page instead of "post list" then your home page layout will be
			"Default Page Layout" below.', 'greenlet' ),
		'id' => "home_template",
		'std' => "12",
		'type' => "images",
	...
	if ( of_get_option('show_topbar') == 1 ) {
		...
		foreach ($tarray as $width) {
			$options[] = array(
				'name' => sprintf( __( 'Topbar Column %s (width = %s) Content:', 'greenlet' ), $col_id, $width ),
	...
	foreach ($harray as $width) {
		$options[] = array(
			'name' => sprintf( __( 'Header Column %s (width = %s) Content:', 'greenlet' ), $col_id, $width ),
	...
	if ( of_get_option('show_semifooter') == 1 ) {
		...
		foreach ($sfarray as $width) {
			$options[] = array(
				'name' => sprintf( __( 'Semi Footer Column %s (width = %s) Content:', 'greenlet' ), $col_id, $width ),
	...
	foreach ($farray as $width) {
		$options[] = array(
			'name' => sprintf( __( 'Footer Column %s (width = %s) Content:', 'greenlet' ), $col_id, $width ),
	...
	$options[] = array(
		...
		'id' => 'show_author',
		'std' => array( 'name' => '1', 'image' => '1', 'bio' => '1' ),
		'type' => 'multicheck',
		'options' => array( 'name' => 'Name', 'image' => 'Avatar', 'bio' => 'Biographical Info' )
	);
	...
	$options[] = array(
		...
		'std' => 'number',
		'type' => 'radio',
		'options' => array( 'simple' => 'Simple', 'number' => 'Numbered', 'ajax' => 'Numbered (Ajax)', 'load' => 'Load More Button', 'infinite' => 'Infinite Scroll' )
	);
